	...
	<table id="pic_table">
		...
		<tbody>
			<tr>
				...
				<td>
					<?php
					echo CHtml::dropDownList('form_zone', '-1', SchoolZone::model()->zoneList);
					?>
				</td>
				<td>
					<form action="/sch/school/addPic" method="post" target="upload_iframe" enctype="multipart/form-data">
...
$(function() {
	$("#add_pic").click(function() {
		...
		tr.find("td:last").append('<a href="javascript:;" class="del">删除</a>');
		tr.find("td:last").find(".upload_info").empty();
	...
	$("#upload_iframe").load(function() {
		...
		$("form").eq(i-1).parent("td").next().find(".upload_info").css("color", c).html(r.msg);
		$("form").eq(i-1).parents("tr:eq(0)").find("input").val("");
		setTimeout(function() {
			uploadPic();
		}, 100);
	});
...
function uploadPic() {
	...
	var formObj = $("form:eq("+i+")"),
		...
		zone = formObj.parents("tr:eq(0)").find("select[name=form_zone]").val();
	
	formObj.find("input[name='Form[title]']").val(picTitle);
	formObj.find("input[name='Form[zone_id]']").val(zone);
	
	formObj.parent("td").next().find(".upload_info").css("color", "green").html("上传中...");
